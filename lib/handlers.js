...
function handlers()
{
	...
	this.compileFromLess = function (file, path, index, isLast, callback) {
		var parser = new(less.Parser)({
			paths: [path.slice(0, path.lastIndexOf('/'))],				// Specify search paths for @import directives
		});

		parser.parse(file, function (e, tree) {
			callback(tree.toCSS({ compress: true }));		// Minify CSS output
		});
	};
	this.uglifyJsOptimize = function (file, path, index, isLast, callback) {
		var minified = uglify.minify(file, {fromString: true});
